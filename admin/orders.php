...
if (isset($_GET['complete']) && $_GET['complete'] == 1) {
  ...
  $dbconn->query("UPDATE cart SET shipped = 1 WHERE id = '{$cart_id}'");
  $_SESSION['success_flash'] = "The order has been completed!";
  header("Location: index.php");
}

$txn_id = sanitize((int)$_GET['txn_id']);
$txnQuery = $dbconn->query("SELECT * FROM transactions WHERE id = '{$txn_id}'");
...
$ids = implode(',',$idArray);
$productQ = $dbconn->query("SELECT i.id as 'id',i.title as 'title', c.id as 'cid', c.category as 'child', p.category as 'parent'
    FROM products i
...
<div class="row">
  <div class="col-md-6">
    ...
    <table class="table table-condensed table-striped table-bordered">
      ...
      <tr>
        <td>Date</td>
        <td><?=preety_time($txn['txn_date']);?></td>
      </tr>
    </table>
  </div>
  <div class="col-md-6">
     ...
     <address>
       <?=$txn['full_name'];?><br />
       <?=$txn['street'];?><br />
       <?=($txn['street1'] != '')?$txn['street1'].'<br />':'';?>
       <?=$txn['city'];?><br />
       <?=$txn['country'];?><br />
     </address>
  </div>
<div class="pull-right">
  <a href="index.php" class="btn btn-large btn-info">Cancel</a>
